...
ftsAppealTransactions <- function(id = NULL) {
  ...
  for (i in 1:length(id)) {
    m = paste("Appeal details:", id[i], "| ")
    cat(m)
    TABLE_NAME = paste0("Appeal_",id[i])

    query_url = paste0(url, id[i], ".json")
    it <- fromJSON(getURL(query_url))
    if (is.data.frame(it)) {
      # write.csv(it, paste(TABLE_NAME, ".csv"), row.names=F)
...
# Scraperwiki wrapper.
runScraper <- function() {
  fts_summary = ftsAppealSummary(year=YEAR)
  ftsAppealTransactions(fts_summary$id)
}
